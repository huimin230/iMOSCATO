...
create.iMOSCATO <- function(sc_count, sc_meta, st_count, loc, cutoff_sample = 100, cutoff_feature = 0.1, 
  ...
  # Check that metadata and count matrix have the same number of cells
  stopifnot(nrow(sc_count) == length(cell_type))
  
  # Check missing cell-type labels
  if (anyNA(cell_type)) {
    stop("Missing values are present in sc_meta$cellType.")
  }
  
  # Sum gene counts within each cell type
  B_sum <- rowsum(sc_count, group = cell_type)
  
  # Number of cells in each cell type
  n_k <- table(cell_type)
  
  # Compute reference matrix b_kj
  B <- sweep(
    B_sum,
    MARGIN = 1,
    STATS = n_k[rownames(B_sum)],
    FUN = "/"
  )
